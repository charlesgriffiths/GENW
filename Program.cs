...
1. Если глобальный игрок в лесу (любом), то лес просто рисовать поверх игрока, специальной иконки не нужно.
Точнее, для простоты кода, просто перерисовать этот тайл еще раз поверх игрока. 
Точнее, это даже не к игроку, а ко всем глобальным объектам относится!
...
2. Честная партия.
2. Видимо, понадобится 2 версии загрузки мира: debug 0 1. Чтобы с партией было интересно бегать и чтобы одновременно 
не тормозить с тестами.
